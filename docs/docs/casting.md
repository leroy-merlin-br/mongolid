...
When you define an attribute to be cast as `DateTime` or `DateTimeImmutable`, Mongolid will load it from database and convert to `DateTime` instance(or `DateTimeImmutable`).
